...
class TenantSRAController extends GetxController {
  ...
  @override
  void dispose() {
    Get.delete<TenantSRAController>();
    super.dispose();
  }
  ...
  Future getMasterData({
    ...
    try {
      ...
      if (pageName.value == RouteNames.kPostHandOverSRAUnit.split('/').last && fieldObj['field'] == 'hutmentid') {
        filter['hutmentid'] = setDefaultData.formData['hutmentid'] ?? '';
      }

      if (pageName.value == RouteNames.kPostHandOverSRAUnit.split('/').last && fieldObj['field'] == 'unitid') {
        filter['unitid'] = setDefaultData.formData['unitid'] ?? '';
      }
      var projection = {};
  ...
  Future setFormData({String? id, int? editeDataIndex, bool? clone, bool canSwitchTab = true}) async {
    ...
    //   setDefaultData.formData['wbscodes'] = response["data"];
    // }
    formLoadingData.value = false;
    update();
  }
  ...
  Future<void> setPaymentInTable(String? id) async {
    /// Ask VATSAL J. for more detail about this comment///
      ...
      case HtmlControls.kAvatarPicker:
        {
          ...
          value = List<FilesDataModel>.from(await IISMethods().uploadFiles(value));
          if (value.isNotEmpty) {
            Map? oldData = IISMethods()
                .encryptDecryptObj((setDefaultData.formData[groupKey][index][key] ?? {})['url'].toString().isNotNullOrEmpty ? setDefaultData.formData[groupKey][index][key] : null);
      ...
      case HtmlControls.kAvatarPicker:
        {
          ...
          value = List<FilesDataModel>.from(await IISMethods().uploadFiles(value));
          if (value.isNotEmpty) {
            Map? oldData = IISMethods().encryptDecryptObj(
                (setDefaultData.masterFormData[groupKey][index][key] ?? {})['url'].toString().isNotNullOrEmpty ? setDefaultData.masterFormData[groupKey][index][key] : null);
  ...
  handleRentCalculation(index) {
    if (setDefaultData.masterFormData['rentdetails'][index]['paymenttypefrequency'] == 2) {
      double rentInMonths = setDefaultData.masterFormData['rentdetails'][index]['rentinmonth'].toString().converttoDouble;
      double noOfMonths = setDefaultData.masterFormData['rentdetails'][index]['noofmonths'].toString().converttoDouble;
      if (setDefaultData.masterFormData['rentdetails'][index]['startdate'].toString().isNotNullOrEmpty) {
        DateTime startDate = DateTime.parse(setDefaultData.masterFormData['rentdetails'][index]['startdate'].toString());

        setDefaultData.masterFormData['rentdetails'][index]['enddate'] = dateConvertIntoUTC(
            DateTime(startDate.year, startDate.month + int.parse(setDefaultData.masterFormData['rentdetails'][index]['noofmonths'].toString()), startDate.day)
                .subtract(const Duration(days: 1)));
      }
      setDefaultData.masterFormData['rentdetails'][index]['totalrent'] = (rentInMonths * noOfMonths).toStringAsFixed(2).converttoDouble;
      setDefaultData.masterFormData['rentdetails'][index]['totalpayable'] =
          (setDefaultData.masterFormData['rentdetails'][index]['totalrent'].toString().converttoDouble).toStringAsFixed(2).converttoDouble;
    }
  }
      ...
      case HtmlControls.kAvatarPicker:
        {
          ...
          if (value.isNotEmpty) {
            ...
            if (key == 'tenantcanceledcheque') {
              AppLoader();
              var response =
                  await IISMethods().listData(userAction: 'addtenantproject', pageName: 'tenantproject', url: '${Config.weburl}readcheque', reqBody: value.first.toJson());
              if (response['status'] == 200) {
  ...
  handleMasterAddButtonClick({String? pagename}) async {
    if (pagename == "managerassign" || pagename == "paymentconfiguration") {
      var url = '${Config.weburl}$pagename${pagename == "paymentconfiguration" ? "/add" : ''}';
  ...
  Future addMasterData({reqData, pageName}) async {
    ...
    if (resBody["status"] == 200) {
      ...
      if (dialogBoxData['dataview'] != 'tab' || dialogBoxData['formfields'].length - 1 == selectedTab.value) {
        ...
        setDefaultData.masterFormData.value = Map<String, dynamic>.from(resBody['data']);
        selectedTab.value++;
      }
      showSuccess(message.value);
    } else {
  ...
  getMasterFormData({String? id, String? documenttype, String? documenttypeid, String? pagename, Map? sort}) async {
    ...
    var reqBody = {
      "paginationinfo": {
        ...
        "filter": {"tenantprojectid": id, "documenttype": documenttype ?? "", "documenttypeid": documenttypeid ?? ""},
        "sort": sort ?? {}
      }
    };
    var response = await IISMethods()
        .listData(userAction: 'list${pageName.value.toString()}', pageName: pageName.value.toString(), url: '${Config.weburl}${pagename.toString()}', reqBody: reqBody);
